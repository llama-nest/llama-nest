...
func (s *Server) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if capture.Model != "" || len(capture.Messages) > 0 {
		...
		if promptTokens > 0 || completionTokens > 0 {
			...
			if err != nil {
				log.Printf("usage capture failed: %v", err)
			}
		}
	}
}
...
func extractUsage(body []byte) (promptTokens int64, completionTokens int64) {
	...
	promptTokens = int64FromAny(raw["prompt_eval_count"])
	completionTokens = int64FromAny(raw["eval_count"])
